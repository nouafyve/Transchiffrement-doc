...
public class GenerationCertificat {
	...
	//		p.waitFor();
			...
			p = Runtime
					.getRuntime()
					.exec("cp vierge.jks "+ip+".jks");
			p.waitFor();
			p = Runtime
					.getRuntime()
					.exec("keytool -importkeystore -srckeystore cles/fakestore.p12 -srcstoretype pkcs12 -srcstorepass 000000 -destkeystore keystores/"+ip+".jks -deststoretype jks -deststorepass 000000 -srcalias 1 -destalias 1");
			p.waitFor();
		}
